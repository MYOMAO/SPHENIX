...
void RMS2018()
{
	...
	TFile *fin = new TFile(ProjectionFile.Data());
...
TitleName = Form("%s_%s_%s_%s",Yeartag.Data(),Typetag.Data(),Methodtag.Data(),recabtag.Data());
	...
	c1->SaveAs(Form("Results/%d/%s.png",year,TitleName.Data()));
	...
	hProjX->Scale(1.0/(YHisBins+1));
	hProjY->Scale(1.0/(XHisBins+1));
	...
	c1->SaveAs(Form("Results/%d/ProjXHis-%s.png",year,TitleName.Data()));
	hProjY->Draw();
	c1->SaveAs(Form("Results/%d/ProjYHis-%s.png",year,TitleName.Data()));
	...
	fout->cd();
	MEHis->Write();
	TF1 *f1 = new TF1("f1","gaus",4.4,5.05);
	MEHis->Fit(f1,"R");
