...
void ReadHis2()
{
	...
	sprintf(inputfile,OutFileMakeFile.Data());
			...
			if(Method ==1){
				...
				if(average > 1) 
				{
					...
					if(Center > LowEThres && Center < HighEThres){
						...
						sprintf(hisname,"pngfiles/%d/Plot_%s_%d-%d.png",year,TitleName.Data(),x,y);
	...
	c22->SaveAs(Form("pngfiles/%d/Single/EnPo.png",year));

	StatPo->Draw("colz");

	c22->Update();

	c22->SaveAs(Form("pngfiles/%d/Single/StatPo.png",year));
