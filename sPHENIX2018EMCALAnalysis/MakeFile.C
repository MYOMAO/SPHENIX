...
void MakeFile(){
	...
	if(recab ==1){
		if(PosFix ==1){
			...
			TFile *fin2 = new TFile(PosFixOut.Data());
			TTree *t2 = (TTree *)fin2->Get("RunxyFix");
			t->AddFriend(t2);
			TH3D *Energyhis = new TH3D("Energyhis","",XBins, Xmin,Xmax,YBins,Ymin,Ymax,EBins,Emin,Emax);
			t->Draw(Form("clus_5x5_recalib.sum_E*%s:beam_2CV_mm_fix + 5*info.hodo_v:beam_2CH_mm_fix - 5*info.hodo_h>>Energyhis",scale.Data()),"good_e");
			//return;
		...
		//	t->Draw(Form("clus_5x5_recalib.sum_E*%s:beam_2CV_mm + 5*info.hodo_v:beam_2CH_mm - 5*info.hodo_h>>Energyhis","good_e",scale.Data()));
			t->Draw(Form("clus_5x5_prod.sum_E*%s:beam_2CV_mm + 5*info.hodo_v:beam_2CH_mm - 5*info.hodo_h>>Energyhis",scale.Data()),"good_e");
	...
	//	t->Draw(Form("clus_5x5_prod.sum_E*%s:beam_2CV_mm_fix + 5*info.hodo_v:beam_2CH_mm_fix - 5*info.hodo_h>>Energyhis",scale.Data()),"good_e");
		t->Draw(Form("clus_5x5_prod.sum_E*%s:beam_2CV_mm_fix + 5*info.hodo_v:beam_2CH_mm_fix - 5*info.hodo_h>>Energyhis",scale.Data()));
	//return;
	cout << "Mean here =" << Energyhis->GetMean() << endl;
	}
	...
	if(debug == 0){
		...
		TH2D *h1= new TH2D("h1","",XBins/5,Xmin+XAd,Xmax+XAd,8,-0.5,7.5);
		TH2D *h2= new TH2D("h2","",YBins/5,Ymin,Ymax,8,-0.5,7.5);

		t->Draw("clus_5x5_prod.average_col:XPosM >> h1",event_sel);
		t->Draw("clus_5x5_prod.average_row:YPosP >> h2",event_sel);
