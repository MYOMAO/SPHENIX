...
void Projection2018()
{
	...
	c22->SaveAs(Form("Results/%d/%s-Normalized.png",year,TitleName.Data()));

	char Outname[512];
	//	int Binwidth;
	double left;
	double right;
	//double xinitial[NTowerX*2-1] = {x1,(x1+x2)/2,x2,(x2+x3)/2,x3,(x3+x4)/2,x4,(x4+x5)/2,x5,(x5+x6)/2,x6,(x6+x7)/2,x7,(x7+x7)/2,x8};
	...
	for(int i=0; i < NTowerX*2-1; i++)
	{
		...
		c23->Update();
		TString OutnameProjX = Form("Results/%d/ProjectionX%d.png",year,i);
		...
		ReverseXAxis(ProjXHis);
		c23->Update();
		OutnameProjX = Form("Results/%d/CombX%d.png",year,i);
		c23->SaveAs(OutnameProjX);
	...
	for(int i=0; i < NTowerY*2-1; i++)
	{
		...
		c22->Update();
		TString OutnameProjY = Form("Results/%d/ProjectionY%d.png",year,i);
		...
		ReverseXAxis(ProjXHis);
		c22->Update();
		OutnameProjY = Form("Results/%d/CombY%d.png",year,i);
		c22->SaveAs(OutnameProjY);
